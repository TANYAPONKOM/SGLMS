...
if ($docId <= 0) {
  ...
  if ($docId <= 0) { echo 'ยังไม่มีเอกสารของคุณ'; exit; }
}
...
if (!$document){ echo 'ไม่พบเอกสาร หรือไม่มีสิทธิ์เข้าถึง'; exit; }

/** ดึงค่ารายช่อง */
$vals = $pdo->prepare("SELECT field_id, value_text FROM document_values WHERE document_id = :id");
$vals->execute([':id'=>$docId]);
$valueMap = [];
foreach ($vals->fetchAll(PDO::FETCH_ASSOC) as $r){ $valueMap[(int)$r['field_id']] = (string)$r['value_text']; }
...
$thaiYear = '';
if ($docDate && preg_match('/^\d{4}/',$docDate)) { $thaiYear = ((int)substr($docDate,0,4) + 543); }
?>
<!DOCTYPE html>
<html lang="th">
...
<body>
  ...
  <main class="page">
    <div class="relative mb-5 h-[84px]">
      <img src="https://i.pinimg.com/474x/bd/55/cc/bd55ccc4416012910a723da8f810658b.jpg" alt="ตราครุฑ (ลายเส้น)"
        class="absolute left-0 top-1/2 -translate-y-1/2" style="height:70px;width:auto" />
      <h1 class="absolute inset-0 flex items-center justify-center font-bold text-2xl">บันทึกข้อความ</h1>
    </div>
    ...
    <div class="mb-2 text-[16px]">
      <div class="flex mb-1.5">
        ...
        <span class="flex-1 underlined"><span class="editable" contenteditable="true"><?=h($hdr_agency)?> โทร.
            ....</span></span>
      </div>
      <div class="flex mb-1.5">
    ...
    <div class="content-block thai-justify indent-first mb-3">
      การนี้ ข้าพเจ้า
      <span contenteditable="true" class="chip"><?=h(($ownerName?:'ชื่อ-นามสกุล').' '.($position?:''))?></span>
      ...
      <span contenteditable="true" class="chip"><?=h($thaiYear ? 'พ.ศ. '.$thaiYear : 'พ.ศ. ....')?></span>
      ในส่วนของภาควิชา<?=h($department ?: '................')?> แผนงานจัดการศึกษาระดับอุดมศึกษา กองทุนพัฒนาบุคลากร
    ...
    <div class="mt-12 text-left text-[16px]">(ผู้ช่วยศาสตราจารย์ ดร.
      ................................)<br />หัวหน้าภาควิชา<?=h($department ?: '................')?></div>
